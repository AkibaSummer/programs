...
struct tarjan {     //tarjan模板
	...
	void addedge(long long u, long long v,long long id) {   //添加边
		e[u].push_back({v,id});
	}
	void dfs(long long u) {       //dfs类似环缩点的写法，两个主要数组dfn[]和low[]
		dfn[u] = low[u] = ++index;
		bool flag=0;
...
void addedge(long long uu,long long vv,long long ww) {
    e[mal].to = vv; e[mal].w = ww; e[mal].nx = head[uu]; head[uu]= mal++;
}

struct node {
    long long v; long long dis;
...
void dijk(long long dis[], long long s) {
    ...
    memset(dis,0x1f,sizeof(long long)*100005);
    node tmp;
